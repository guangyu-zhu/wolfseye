...
<script type="text/javascript">
var index = '<%=index%>';
function openImageEditWindow(id,path,name){
	$(window.parent.document).find("#dialog_crop_image").attr('href','${contextPath}/plugin/upload/form_image_crop.jsp?index=<%=index%>&id='+id+'&w=<%=(int)w%>&h=<%=(int)h%>&path='+path+'&name='+name.replace("\.","-"));
...
<script src="${contextPath}/admin/assets/scripts/custom/form-fileupload.js"></script>
<script type="text/javascript">
jQuery(document).ready(function() {
   ...
   $.post('${contextPath}/global/findFiles.do',{
			...
			for (var i in files) { 
				...
				var block = $('<tr class="template-upload fade in" data="'+file.name+'"><td><span class="preview"><img id="'+imgId+'" width="'+file.width+'" height="'+file.height+'" src="${contextPath}/upload/files/'+file.path+'/'+file.name+'?id='+Math.random()+'"></span></td><td><p class="name">'+file.type+'</p></td><td><p class="size">'+file.size+'</p></td><td class="btn-group" <%=isView?"style=\"display:none;\"":""%>><button '+file.disabled+' class="btn purple edit btn-sm" onclick="return openImageEditWindow(\''+imgId+'\',\''+file.path+'\',\''+file.name+'\');"><i class="fa fa-external-link"></i><span> 编辑图片</span> '+file.disabledMsg+'</button><button class="btn red remove btn-sm" onclick="return removeImage(this,\''+file.path+'\',\''+file.name+'\');"><i class="fa fa-trash-o"></i><span> 删除图片</span></button></td></tr>');
				$('.files').append(block);
			}
			FormFileUpload.resetIframe();
	});
