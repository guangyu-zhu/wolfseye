package com.yuyue.web.action;

import java.text.DateFormat;
import java.text.MessageFormat;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.ResourceBundle;

import javax.servlet.http.HttpServletRequest;

import org.apache.struts.action.ActionMapping;

import com.yuyue.cu.core.abstractor.AbstractCriteriaBuilder;
import com.yuyue.cu.core.inf.SimpleObject;
import com.yuyue.cu.core.web.action.abstractor.AbstractBaseAction;
import com.yuyue.util.AdminPageTab;
import com.yuyue.web.criteria.PrdCriteriaBuilder;
import com.yuyue.web.jo.PrdJo;

public class PrdAction extends AbstractBaseAction {
	
	private DateFormat df1 = new SimpleDateFormat("yyyy-MM-dd"); 
	private DateFormat df2 = new SimpleDateFormat("yyyy-MM-dd hh:mm"); 

	// GLOBAL CONFIG BEGIN
	@Override
	public AdminPageTab getPageTab() {
		return AdminPageTab.PRD;
	}
	@Override
	public AbstractCriteriaBuilder getPageCriteriaBuilder(
			HttpServletRequest request) {
		return new PrdCriteriaBuilder(request);
	}
	@Override
	public boolean validateAuthorization(ActionMapping mapping,
			HttpServletRequest request) {
		return isAdminLoginAccount(mapping,request) == Boolean.TRUE;
	}
	// GLOBAL CONFIG END
	
	// LIST PAGE BEGIN
	@Override
	public long getListPagePaginationRecordAmount() {
		return 20;
	}
	@Override
	public void buildListPageCriteria(HttpServletRequest request,
			AbstractCriteriaBuilder builder) throws Exception {
		String s = request.getParameter("s");
		if(s != null){
			builder.beginBracket();
			builder.addLkCriteria("hold_percentage", LABEL_LIKE+s+LABEL_LIKE);
			builder.endBracket();
		}
		builder.addOrderDesc("sortId");
		builder.addOrderDesc("updateDate");
	}
	// LIST PAGE END
	
	// DETAIL PAGE BEGIN
	// ... nothing so far
	// DETAIL PAGE END
	
	// SAVE PAGE BEGIN
	@Override
	public String getCreateSuccessMessage(HttpServletRequest request, SimpleObject object) {
		ResourceBundle resourceBundle = getInternalResourceBundle(request);
		return MessageFormat.format(resourceBundle.getString("msg.create.success"),"");
	}
	private void handleRealAlertTime(PrdJo prdJo) {
		Date showDate = prdJo.getShowDate();
		String yyyyMMdd = df1.format(showDate);
		String _showDate = yyyyMMdd+" 08:30";
		try{
			prdJo.setShowDate(df2.parse(_showDate));
		}catch(Exception e){
			throw new RuntimeException(e);
		}
	}
	@Override
	public String getUpdateSuccessMessage(HttpServletRequest request,
			SimpleObject object) {
		ResourceBundle resourceBundle = getInternalResourceBundle(request);
		return MessageFormat.format(resourceBundle.getString("msg.update.success"),"");
	}
	@Override
	public void buildCreatePageCriteria(HttpServletRequest request, SimpleObject object, AbstractCriteriaBuilder builder) throws Exception {
		PrdJo obj = (PrdJo)object;
		handleRealAlertTime(obj);
		obj.setCreateDate(new Date());
		obj.setUpdateDate(new Date());
	}
	@Override
	public void buildUpdatePageCriteria(HttpServletRequest request, SimpleObject object, AbstractCriteriaBuilder builder) throws Exception {
		PrdJo obj = (PrdJo)object;
		handleRealAlertTime(obj);
		obj.setUpdateDate(new Date());
	}
	// SAVE PAGE END
	
	// REMOVE PAGE BEGIN
	@Override
	public String getRemoveSuccessMessage(HttpServletRequest request, SimpleObject object) {
		ResourceBundle resourceBundle = getInternalResourceBundle(request);
		return MessageFormat.format(resourceBundle.getString("msg.remove.success"),"");
	}
	// REMOVE PAGE END

}
